...
## Task 3: Thay edge-tts bằng OmniVoice — Voice Cloning TTS

### Tổng quan

Chuyển TTS engine từ **edge-tts** (Microsoft Text-to-Speech, giọng preset) sang **OmniVoice** (open-source, zero-shot voice cloning, 600+ ngôn ngữ).

**Concept**: User chuẩn bị sẵn các file âm thanh mẫu (3-10 giây) trong thư mục `voice_clones/`. Trên UI, dropdown sẽ liệt kê các file voice clone. Khi dubbing, backend dùng OmniVoice để clone giọng từ file tham chiếu.

### Architecture

```
voice_clones/
  ├── narrator.wav          (file âm thanh mẫu)
  ├── narrator.txt          (transcript tương ứng)
  ├── female_host.wav
  ├── female_host.txt
  └── deep_male.wav
      deep_male.txt
...
Mỗi voice clone gồm 2 file:
- `.wav` — Audio mẫu (3-10 giây, chất lượng cao)
- `.txt` — Transcript chính xác của audio mẫu (OmniVoice cần để align)
...
Service mới xử lý TTS bằng OmniVoice:
...
def get_tts_model():
    """Lazy-load OmniVoice model (tốn VRAM, chỉ load 1 lần)"""
    global _model
    if _model is None:
        _model = OmniVoice()  # Auto-detect GPU
    return _model

def list_voice_clones() -> list[dict]:
    """Liệt kê các voice clone có sẵn từ thư mục voice_clones/"""
    clones = []
    for wav_file in VOICE_CLONES_DIR.glob("*.wav"):
        txt_file = wav_file.with_suffix(".txt")
        ref_text = txt_file.read_text(encoding="utf-8").strip() if txt_file.exists() else ""
        clones.append({
            "id": wav_file.stem,
...
async def synthesize_line(
    ...
    """Sinh audio cho 1 dòng subtitle bằng OmniVoice voice cloning"""
    model = get_tts_model()
    clone = _get_clone_by_id(voice_id)
...
```python
# GET /api/v1/tts/voices — Liệt kê voice clones
# POST /api/v1/dubbing — Chạy dubbing pipeline
```

##### [MODIFY] [config.py](file:///e:/AI-Video_Localizer/backend/core/config.py)

Thêm config cho voice clones directory:
...
Pipeline dubbing hoàn chỉnh:
1. Đọc SRT → duyệt từng entry
2. OmniVoice: Sinh audio cho mỗi line (voice cloning)
3. FFmpeg atempo: Điều chỉnh tốc độ nếu TTS dài hơn segment
4. FFmpeg: Tạo TTS audio track đặt đúng timestamp
5. Dynamic ducking: Giảm volume gốc chỉ khi có TTS
6. Mix 2 tracks → final video
...
### Task 1
- Mở video → bật "Chọn vùng OCR" → phải thấy frame video (snapshot), không phải màn đen
- Kéo chọn vùng → thấy rectangle trên snapshot → restore video sau khi chọn xong

### Task 2
- Load video + SRT → click vào dòng sub bất kỳ → video pause tại StartTime
- Video đang phát → click dòng sub → video dừng ngay
- Double-click → video seek + play (preview)
